...
export default function TreatmentsList({
  ...
  patientId,
}: { treatments: any[]; patientId: string }) {
  ...
  if (treatments.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="mt-2 text-lg font-medium text-gray-900">Nenhum tratamento encontrado</h3>
        <p className="mt-1 text-sm text-gray-500">Este paciente ainda não possui tratamentos registrados.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button variant="outline" size="sm" onClick={refreshTreatments} disabled={isRefreshing}>
      ...
      {treatments.map((treatment) => {
        ...
        const hasPaid = treatment.treatment_payment && treatment.treatment_payment.payment_date
